...
import threading
import uuid
import gdown  # 添加gdown用于从Google Drive下载
...
# ==========================
GOOGLE_DRIVE_FOLDER_URL = "https://drive.google.com/drive/folders/1I8TGzkjrJ3lqEHpbU8dlX2riyWBm9RmQ?usp=drive_link"
LOCAL_DATA_DIR = "./syphu-china-model"
...
def download_from_google_drive():
    """下载Google Drive文件夹中的所有文件"""
    try:
        # 创建本地目录
        os.makedirs(LOCAL_DATA_DIR, exist_ok=True)

        st.info("🔄 开始从Google Drive下载数据...")
        ...
        st.success(f"✅ 数据下载完成！文件保存在: {LOCAL_DATA_DIR}")
        return True

    except Exception as e:
        st.error(f"❌ 下载失败: {str(e)}")
        st.info("💡 备用方案：请手动下载文件并放置在项目目录的 'syphu-china-model' 文件夹中")
        return False


def check_and_download_data():
    """检查数据是否存在，如果不存在则下载"""
    if not os.path.exists(LOCAL_DATA_DIR) or not any(os.scandir(LOCAL_DATA_DIR)):
        st.warning("📁 未找到本地数据文件")
        if st.button("📥 从Google Drive下载数据"):
...
required_keys = [
    ...
    'data_source', 'google_drive_connected'
]
for k in required_keys:
    if k not in st.session_state:
        if k in ['analyzed_images', 'current_data_analysis', 'ml_models', 'experiment_records', 'gene_sets',
            ...
            st.session_state[k] = {}
        elif k in ['google_drive_connected']:
...
# ==========================
# File scanning (cached) - 修改为支持Google Drive
# ==========================
@st.cache_data(ttl=3600)
def scan_research_data(base_dir=LOCAL_DATA_DIR):
    try:
        # 检查数据是否存在
...
if not st.session_state.get('research_files') or not any(st.session_state.research_files.values()):
    if check_and_download_data():
        st.session_state.research_files = scan_research_data()
    else:
        st.session_state.research_files = {'images': [], 'data': [], 'results': [], 'sequences': []}
...
# ==========================
# UI: Banner & Sidebar - 添加Google Drive集成
# ==========================
st.markdown("""
...
<div class="google-drive-notice">
    <h4>🌐 Google Drive 集成</h4>
    <p>数据来源: <a href="{GOOGLE_DRIVE_FOLDER_URL}" style="color: white; text-decoration: underline;" target="_blank">Google Drive 文件夹</a></p>
    <p>本地目录: {LOCAL_DATA_DIR}</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # 修改后
    p_value_threshold = st.selectbox("P-value Threshold", [0.05, 0.01, 0.001], index=0, key="sidebar_pval_thresh")
...
# ==========================
# Page: Project Overview - 添加数据状态检查
# ==========================
...
if page == "🏠 Project Overview":
    ...
    # 数据状态检查
    if not any(research_files.values()):
        st.warning("⚠️ 未找到数据文件。请点击侧边栏的 'Download from Google Drive' 按钮下载数据。")
    else:
        st.success(f"✅ 已加载 {total_files} 个文件")
...
# ==========================
# Page: Data Explorer - 添加数据下载提示
# ==========================
elif page == "🔬 Data Explorer":
    st.markdown('<div class="main-header">Advanced Data Explorer</div>', unsafe_allow_html=True)

    if not research_files.get('data'):
        st.info("📊 No data files found. Please download data from Google Drive using the button in the sidebar.")
        st.markdown("### 🎯 数据下载说明")
        st.write("""
        1. 点击侧边栏的 **'Download from Google Drive'** 按钮
        2. 等待数据下载完成
        3. 刷新页面或点击 **'Rescan Files'** 重新扫描文件
        """)

        if st.button("📥 立即下载数据", type="primary"):
            if download_from_google_drive():
                st.session_state.research_files = scan_research_data()
                safe_rerun()
    else:
        # 原有的Data Explorer代码保持不变
